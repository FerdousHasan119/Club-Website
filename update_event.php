  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(mysqli_num_rows($run)>0){
        while($row = $run->fetch_assoc()){
          $html = $html. "<tr><td>" . $row["name"] . "</td><td>" . "<img src='event img/" . $row["img"] . "' width = 300px class='rounded mx-auto d-block'>" . "</td><td>" . $row["location"] . "</td><td>" . $row["e_time"] . "</td><td>". $row["details"] . "</td><td>". $row["participant"] . "</td>
          <td>". '<a  href="update_eventprocess.php?id= '.$row['id'].'" class="btn btn-lg btn-block btn btn-outline-info"> Update</a>'.'<button type="submit" id="' . $row["id"] .'" class="del btn btn-outline-danger">Delete</button>' ."</td></tr>";
            $flag = 1;
...
<body>
    ...
    <form class="form" action="update_event.php" method="POST">
     ...
     <button id="btnSubmit" onclick="window.location.href='update_event.php'" type="button" class="btn btn-lg btn-block btn-info"><i class="fa-solid fa-list"></i> See All</button>
     <?php if ($msg) { ?>                
                <div class="row">
                    <div class="col-md-12">
