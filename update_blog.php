...
<?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $q =  "SELECT * FROM `blog` WHERE  `$cat` LIKE '%$search%';";
            $run = mysqli_query($con, $q);

            if(mysqli_num_rows($run)>0){
                while($row = $run->fetch_assoc()){
                    $html = $html. "<tr><td>" . $row["title"] . "</td><td>" . $row["name"] . "</td><td>" . $row["email"] . "</td><td>". $row["time"] . "</td><td>". $row["content"] . "</td>
                    <td>". '<a  href="update_blogprocess.php?id= '.$row['id'].'" class="btn btn-lg btn-block btn btn-outline-info"> Update</a>'.'<button type="submit" id="' . $row["id"] .'" class="del btn btn-outline-danger">Delete</button>' ."</td></tr>";
                    $flag = 1;
                }
            }
        }
